...
void netspace_table::add_node(const netnode& node) noexcept {
...
netspace_table::iterator netspace_table::find_addr(const netspace_addr& addr) noexcept {
	return std::find_if(std::begin(m_table), std::end(m_table), [&addr](netnode& v) {
...
netspace_table::iterator netspace_table::find_host(const std::string& host) noexcept {
	return std::find_if(std::begin(m_table), std::end(m_table), [&host](netnode& v) {
		return v.hostname() == host;
	});
}

netspace_table::iterator netspace_table::find_type(netnode_type nt) {
	return std::find_if(std::begin(m_table), std::end(m_table), [nt](netnode& v) {
		return v.type() == nt;
	});
}

netspace_table::iterator netspace_table::find_type(netnode_type nt, iterator start) {
	return std::find_if(start, std::end(m_table), [nt](netnode& v) {
		return v.type() == nt;
	});
}


netnode& netspace_table::at(size_type index) {
